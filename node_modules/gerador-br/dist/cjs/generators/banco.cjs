...
var dbBancos = require('../data/db-bancos.cjs');
 ...
 * }
 */
const banco = function bank(code) {
  let bankRand = '';
  if (code) {
    bankRand = dbBancos.find((bnk) => bnk.code === parseInt(code, 10));
    if (!bankRand) {
      // console.log("Banco não encontrado")
      bankRand = getRandomBank(dbBancos);
    }
  } else {
    bankRand = getRandomBank(dbBancos);
  }
 ...
 */
const listaCodigosBancos = function () {
  return dbBancos.map((bank) => bank.code);
};

exports.banco = banco;
exports.listaCodigosBancos = listaCodigosBancos;
